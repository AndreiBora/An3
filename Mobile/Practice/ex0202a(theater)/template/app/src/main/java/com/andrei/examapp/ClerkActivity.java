...
public class ClerkActivity extends AppCompatActivity{
    ...
    private FloatingActionButton btnMarkAv;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_clerk);
        setTitle("Theater section");

        recyclerView = findViewById(R.id.rv_clerk);
        progressBar = findViewById(R.id.progress_bar);
        btnMarkAv = findViewById(R.id.btn_mark_available);
        ...
        btnMarkAv.setOnClickListener(v->{
            viewModel.markAvailable();
        });
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == ADD_ENTITY_REQUEST && resultCode == RESULT_OK){
            GenericEntity entity = (GenericEntity) data.getSerializableExtra(AddEntityActivity.EXTRA_ADD_ENTITY);
            progressBar.setVisibility(View.VISIBLE);
